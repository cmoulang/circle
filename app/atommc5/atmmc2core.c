...
#elif (PLATFORM==PLATFORM_AVR || PLATFORM==PLATFORM_RPI)
...
// AVR doesn't currently have spare port so don't compile it !
			...
			} else if (received == CMD_SET_CFG_BYTE) // write config byte
			{
				...
				logMessage("CMD_SET_CFG_BYTE %x %x", (unsigned int)EE_SYSFLAGS, (unsigned int)configByte);
