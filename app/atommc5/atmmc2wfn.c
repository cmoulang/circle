...
void at_initprocessor(void) {
	...
	logMessage("f_mount(%s) %x", DRIVE, result);

	for (int i = 0; i < 4; i++) {
		fildata[i].obj.fs = NULL;
	}

	result = f_chdrive(DRIVE);
	logMessage("f_chdrive(%s): %x", DRIVE, result);
...
void GetWildcard(void) {
	...
	if (WildPos > -1) {
		if (LastSlash > -1) {
			...
			strncpy(WildPattern, (const char*) &globalData[LastSlash + 1],
			WILD_LEN);
		} else {
...
void wfnDirectoryOpen(void) {
	...
	GetWildcard();
	logMessage("wfnDirectoryOpen %s", (const char*) globalData);
...
void wfnDirectoryRead(void) {
	logMessage("wfnDirectoryRead %s", (const char*) globalData);
	...
	while (1) {
		...
		if (Match) {
			...
			memcpy(&globalData[len + 2], (void*) &(filinfo->fsize),
					sizeof(DWORD));
...
void wfnSetCWDirectory(void) {
	logMessage("wfnSetCWDirectory %s", (const char*) globalData);
...
void wfnRename(void) {
	...
	const XCHAR* to = (const XCHAR*) (globalData
			+ strlen((const char*) globalData) + 1);
...
void wfnFileOpenRead(void) {
	logMessage("wfnFileOpenRead %s", (const char*) globalData);

	res = fileOpen(FA_OPEN_EXISTING | FA_READ);
	if (filenum < 4) {
		FILINFO *filinfo = &filinfodata[filenum];
		FRESULT fres = f_stat((const char*) globalData, filinfo);
		logMessage("f_stat %s = %d", (const char*) globalData, fres);
...
void wfnFileWrite(void) {
	...
	WriteDataPort(
	STATUS_COMPLETE | f_write(fil, (void*) globalData, globalAmount, &written));
}
